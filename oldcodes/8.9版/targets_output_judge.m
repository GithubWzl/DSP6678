...
board_belong=zeros(100,1);   %属于哪个小子板

targets_output_xy=zeros(100,2);  %局部坐标系坐标

%物理子板划线分界坐标
border_x=[3.5,6.5,9.5,12.5,15.5,18.5,21.5,24.5,27.5,30.5];   
border_y=[2.5,4.5,6.5,8.5,10.5,12.5];

count_x=0;  %第几列
count_y=0;  %第几行
%   此处显示详细说明
    for i=1:100    %循环遍历每个目标输出点
        
        for j=1:10   %判断目标输出点的X范围
     ...
     big_board_belong=count_x + (count_y-1)*10;    %判断在哪个物理子板上
