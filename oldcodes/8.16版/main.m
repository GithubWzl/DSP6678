...
avg_method=1;%1：平均平面度计算每个子区平面度的平均值；2：平均平面度计算全部点的平面度
cnt_output=101;%目标输出点个数

%数据划分
targets_valid_origin=data_targets_on_boards(:,4);
...
targets_delta_pt1nk=data_test(:,10);
targets_delta_pt2nk=data_test(:,11);

targets_output_XY=data_output(1:cnt_output,6:7);%目标输出点坐标
...
%子板划分
[targets_little_board,little_board_valid]=board_div(targets_outlier,board_cnt,targets_per_board,targets_per_phy_board,targets_valid_origin);%targets_little_board大小为480*3，第一列为小子板编号，第二列为靶标整体编号，第三列为靶标有效信号。little_board_valid大小为120*1，表示小子板有效信号
...
%平面度参数计算
[board_para_pt1,board_para_pt2,board_para_pt1_avg,board_para_pt2_avg] = flatness_calculation(targets_little_board,targets_xy,targets_Delta_pt1nk,targets_Delta_pt2nk,little_board_valid,avg_method,targets_XY,targets_outlier);
...
A=sum(abs(delta_pt1_output-delta_pt2_output));
B=board_para_pt2_avg-board_para_pt1_avg;
