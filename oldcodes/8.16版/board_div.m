function [targets_little_board,little_board_valid]=board_div(targets_outlier,board_cnt,targets_per_board,targets_per_phy_board,targets_valid_origin)
...
for m=1:board_cnt/2
    ...
    targets6=1-targets_outlier((m-1)*targets_per_board*1.5+1:m*targets_per_board*1.5);
    targets8=[targets6(1);targets6(2);targets6(3);targets6(4);targets6(3);targets6(4);targets6(5);targets6(6)] & targets_valid_origin(2*(m-1)*targets_per_board+1:2*m*targets_per_board);
    targets8=double(targets8);
    targets_valid(2*(m-1)*targets_per_board+1:2*m*targets_per_board)=targets8;
    if sum(targets8(1:targets_per_board))==4
        little_board_valid(2*m-1)=1;
    elseif sum(targets8(1:targets_per_board))==3
        little_board_valid(2*m-1)=1;
    else
        little_board_valid(2*m-1)=0;
        fprintf('子区%d野值过多，无效\n',2*m-1);
    end
    if sum(targets8(targets_per_board+1:2*targets_per_board))==4
        little_board_valid(2*m)=1;
    elseif sum(targets8(targets_per_board+1:2*targets_per_board))==3
        little_board_valid(2*m)=1;
    else
